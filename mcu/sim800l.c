...
unsigned char code TC_BAUD[] = "AT+IPR=19200";			// 波特率
...
void tcp_9_sendtext()
{
	...
	while(1)
	{
		send_string(TC_TCP_SENDTEXT);

		send_byte(0x0d);
		send_byte(0x0a);
		Delay1ms(20); // 在调试的时候需要把延迟加大，服务器向串口发送'>'字符串，才会向TCP服务器发送数据
						// 连接模块的时候可以适当调小
		P0 = 0x30;	 // 0011 0000
	...
	while(1)
	{
		for(j = 14;j < strlen(t_datas); j++)	 // 16 跳过短信前面的无效字符从长度开始发
		{
			send_byte(t_datas[j]);
		}
